...
function addCols( $tab )
{
	...
	foreach ($tab as $lib => $pos) // Reemplaza each() por foreach()
	{
...
function lineVert( $tab )
{
	...
	reset($colonnes); // Aunque reset() es opcional con foreach, puede quedarse si lo prefieres
	$maxSize = 0;
	
	foreach ($colonnes as $lib => $pos) // Reemplaza each() por foreach()
	{
		$texte = $tab[$lib];
		$longCell  = $pos - 2;
